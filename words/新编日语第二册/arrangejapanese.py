...
def handleComponents(components):
	...
	japanese = components[0].strip()
	kana = components[1].strip()
	chinese = components[4].strip()
	classname = components[5].strip()
	# isHiragana = components[5]
	types = components[3].split(u'，')
	tones = components[2].split(u'，')
	tone1 = -1
	tone2 = -1
	tone3 = -1
	if len(tones) >= 1:
	...
	print japanese + "," + kana  + "," +  chinese + "," + type1 + "," + type2 + "," + type3 + "," + "%s,%s,%s"%(tone1,tone2,tone3)  + "," + hira  + "," + classname

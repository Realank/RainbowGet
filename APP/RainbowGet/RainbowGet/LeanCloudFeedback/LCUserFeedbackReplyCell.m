...
- (void)setupWithFeedbackReply:(LCUserFeedbackReply *)reply {
    ...
    switch (reply.contentType) {
        case LCContentTypeText:
            [self.contentView insertSubview:self.bubbleView belowSubview:self.textLabel];
            
            break;
        case LCContentTypeImage:
            [self.contentView insertSubview:self.bubbleView belowSubview:self.imageView];
            break;
    }
...
- (UIView *)bubbleView {
    if (_bubbleView == nil) {
        ...
        _bubbleView.layer.cornerRadius = 7;
        _bubbleView.layer.borderWidth = 0.3;
        _bubbleView.layer.borderColor = [UIColor lightGrayColor].CGColor;
    }
    return _bubbleView;
}
...
- (void)configuareCellWithFeedbackReply:(LCUserFeedbackReply *)reply {
    ...
    [self changeBubbleColorForFeedbackReply:reply];
...
- (void)layoutSubviews {
    ...
    CGRect bubbleFrame = [self bubbleContentFrame];
...
- (void)changeBubbleColorForFeedbackReply:(LCUserFeedbackReply *)reply {
	if (reply.type == LCReplyTypeDev) {
        self.bubbleView.backgroundColor = [UIColor whiteColor];
        self.textLabel.textColor = [UIColor darkGrayColor];
	} else {
        self.bubbleView.backgroundColor = [ThemeColor currentColor].tintColor;;
        self.textLabel.textColor = [ThemeColor currentColor].foreColor;;
	}
}
